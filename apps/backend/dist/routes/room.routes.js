...
router.get("/:code", auth_middleware_1.authMiddleware, async (req, res) => {
...
router.post("/:code/join", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
...
router.post("/:code/leave", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
...
router.post("/:code/ready", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
        ...
        (0, sockets_1.emitToRoomNamespace)(roomCode, "room:player_ready", {
            roomCode,
            playerId: userId,
            isReady,
        });
...
router.patch("/:code/team", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
        ...
        (0, sockets_1.emitToRoomNamespace)(roomCode, "room:team_changed", {
            roomCode,
            playerId: userId,
            team: nextTeam,
        });
...
router.post("/:code/start", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
        ...
        const game = await gameManager_1.gameManager.createGame({
        ...
        const orderedPlayers = [...startedRoom.players].sort((a, b) => a.joinedAt.getTime() - b.joinedAt.getTime());
        const handsSnapshot = (startedRoom.game?.handsSnapshot ?? {});
        ...
        (0, sockets_1.emitToRoomNamespace)(roomCode, "room:game_starting", {
            roomCode,
            gameId: game.id,
            gameState,
        });
