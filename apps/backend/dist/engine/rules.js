...
exports.getWinner = exports.isGameOver = exports.getValidMoves = exports.applyDeclare = exports.applyAsk = exports.isValidDeclare = exports.isValidAsk = void 0;
...
const updateHandCounts = (players, hands) => {
    return players.map((player) => ({
        ...player,
        handCount: hands[player.id]?.length ?? 0,
    }));
};
const getPlayer = (gameState, playerId) => {
    return gameState.players.find((player) => player.id === playerId);
};
const getOpponentTeam = (team) => (team === "TEAM_A" ? "TEAM_B" : "TEAM_A");
const hasExactCard = (hand, card) => {
    const code = (0, deck_1.cardToCode)(card);
    return hand.some((ownedCard) => (0, deck_1.cardToCode)(ownedCard) === code);
};
const removeExactCard = (hand, card) => {
    const cardCode = (0, deck_1.cardToCode)(card);
    let removed = false;
    return hand.filter((ownedCard) => {
        if (!removed && (0, deck_1.cardToCode)(ownedCard) === cardCode) {
            removed = true;
            return false;
        }
        return true;
...
const getTeamPlayerIds = (gameState, team) => {
    return gameState.players.filter((player) => player.team === team).map((player) => player.id);
};
const teamHasAnyCardFromHalfSuit = (gameState, team, halfSuit) => {
    const playerIds = getTeamPlayerIds(gameState, team);
    return playerIds.some((playerId) => (gameState.hands[playerId] ?? []).some((card) => card.halfSuit === halfSuit));
};
const getActualHalfSuitHolders = (gameState, halfSuit) => {
    ...
    for (const player of gameState.players) {
        ...
        for (const card of hand) {
            if (card.halfSuit === halfSuit) {
                holderMap[(0, deck_1.cardToCode)(card)] = player.id;
            }
        }
    }
...
const selectNextTurnFromTeam = (gameState, team) => {
...
const isValidAsk = (gameState, askingPlayerId, targetPlayerId, card) => {
    if ((0, exports.isGameOver)(gameState)) {
...
exports.isValidAsk = isValidAsk;
const isValidDeclare = (gameState, declaringPlayerId, declaration) => {
    if ((0, exports.isGameOver)(gameState)) {
        return { valid: false, reason: "Game is already over." };
    }
    ...
    const requiredCards = (0, deck_1.getHalfSuitCards)(declaration.halfSuit).map((card) => (0, deck_1.cardToCode)(card));
...
exports.isValidDeclare = isValidDeclare;
const applyAsk = (gameState, askingPlayerId, targetPlayerId, card) => {
    const validation = (0, exports.isValidAsk)(gameState, askingPlayerId, targetPlayerId, card);
    if (!validation.valid) {
...
exports.applyAsk = applyAsk;
const applyDeclare = (gameState, declaringPlayerId, declaration) => {
    const validation = (0, exports.isValidDeclare)(gameState, declaringPlayerId, declaration);
    if (!validation.valid) {
    ...
    const requiredCodes = (0, deck_1.getHalfSuitCards)(declaration.halfSuit).map((card) => (0, deck_1.cardToCode)(card));
    const isCorrect = requiredCodes.every((code) => declaration.cardHolders[code] && declaration.cardHolders[code] === actualHolders[code]);
    ...
    for (const cardCode of requiredCodes) {
        ...
        const card = (0, deck_1.codeToCard)(cardCode);
    ...
    if ((0, exports.isGameOver)(stateAfterDeclare)) {
        return {
            ...stateAfterDeclare,
            status: "FINISHED",
        };
...
exports.applyDeclare = applyDeclare;
const getValidMoves = (gameState, playerId) => {
    const player = getPlayer(gameState, playerId);
    if (!player) {
    ...
    for (const opponent of gameState.players.filter((candidate) => candidate.team !== player.team)) {
        for (const opponentCard of gameState.hands[opponent.id] ?? []) {
            if (heldHalfSuits.has(opponentCard.halfSuit) &&
                !hasExactCard(playerHand, opponentCard) &&
                !askableCards.some((card) => (0, deck_1.cardToCode)(card) === (0, deck_1.cardToCode)(opponentCard))) {
                askableCards.push(opponentCard);
            }
        }
    }
    const declarableHalfSuits = HALF_SUITS.filter((halfSuit) => teamHasAnyCardFromHalfSuit(gameState, player.team, halfSuit));
    return {
        canAsk: gameState.currentTurnPlayerId === playerId && !(0, exports.isGameOver)(gameState),
        askableCards,
        declarableHalfSuits,
    };
};
